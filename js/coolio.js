...
var mobileExperience = {
    ...
    setLines : function(){
        ...
        this.resize = function(){
            mobileExperience.canvas.width = window.innerWidth;
            mobileExperience.canvas.height = window.innerHeight;
            ctx.fillStyle = color;
            ctx.lineWidth = .1;
            ctx.strokeStyle = color;
        };
        ...
        var mousePosition = {
            x: 30 * mobileExperience.canvas.width / 100,
            y: 30 * mobileExperience.canvas.height / 100
        };
        ...
        function Dot(){
            this.x = Math.random() * mobileExperience.canvas.width;
            this.y = Math.random() * mobileExperience.canvas.height;
            this.vx = -.5 + Math.random();
            this.vy = -.5 + Math.random();
            this.radius = Math.random();
        ...
        Dot.animate = function(){
            ...
            for(i = 0; i < dots.nb; i++){
                ...
                if(dot.y < 0 || dot.y > mobileExperience.canvas.height){
                    dot.vx = dot.vx;
                    dot.vy = - dot.vy;
                }
                else if(dot.x < 0 || dot.x > mobileExperience.canvas.width){
                    dot.vx = - dot.vx;
                    dot.vy = dot.vy;
                }
                dot.x += dot.vx;
        ...
        function createDots(){
            ...
            ctx.clearRect(0, 0, mobileExperience.canvas.width, mobileExperience.canvas.height);
        ...
        $('#experience').on('mousemove mouseleave', function(e){
            ...
            if(e.type == 'mouseleave'){
                mousePosition.x = mobileExperience.canvas.width / 2;
                mousePosition.y = mobileExperience.canvas.height / 2;
            }
        });
    ...
    destroy : function() {
        ...
        if(experience.resize){
            $(window).on('resize', mobileExperience.resize);
        }
